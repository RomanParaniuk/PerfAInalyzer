...
You run every **`startup_initialization`** work unit. **Static analysis only** —
...
- **This stage's units**: `startup_initialization--p*`
...
- **Digest artifact**: `04g-startup.md`
...
- Default: analyze **all** of this stage's units. Delete existing
  `results/startup_initialization--*.json` files first (never `workplan.json`, never
...
Overwrite `04g-startup.md` (a human summary; the JSONs stay the machine input for
`/perf-report`):

```markdown
---
stage: analysis-execution-startup
...
# Startup & Initialization Findings — <target basename>
