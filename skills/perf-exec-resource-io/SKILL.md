...
You run every **`resource_io_efficiency`** work unit. **Static analysis only** —
...
- **This stage's units**: `resource_io_efficiency--p*`
...
- **Digest artifact**: `04c-resource-io.md`
...
- Default: analyze **all** of this stage's units. Delete existing
  `results/resource_io_efficiency--*.json` files first (never `workplan.json`, never
...
Overwrite `04c-resource-io.md` (a human summary; the JSONs stay the machine input for
`/perf-report`):

```markdown
---
stage: analysis-execution-resource-io
...
# Resource & I/O Efficiency Findings — <target basename>
