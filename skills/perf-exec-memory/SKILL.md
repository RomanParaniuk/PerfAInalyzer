...
You run every **`memory_allocation`** work unit. **Static analysis only** —
...
- **This stage's units**: `memory_allocation--p*`
...
- Default: analyze **all** of this stage's units. Delete existing
  `results/memory_allocation--*.json` files first (never `workplan.json`, never
...
Overwrite `04e-memory.md` (a human summary; the JSONs stay the machine input for
`/perf-report`):

```markdown
---
stage: analysis-execution-memory
...
# Memory & Allocation Findings — <target basename>
