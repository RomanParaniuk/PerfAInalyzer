...
You run every **`algorithmic_complexity`** work unit. **Static analysis only** —
...
- **This stage's units**: `algorithmic_complexity--p*`
...
- **Digest artifact**: `04b-complexity.md`
...
- Default: analyze **all** of this stage's units. Delete existing
  `results/algorithmic_complexity--*.json` files first (never `workplan.json`, never
...
Overwrite `04b-complexity.md` (a human summary; the JSONs stay the machine input for
`/perf-report`):

```markdown
---
stage: analysis-execution-complexity
...
# Algorithmic Complexity Findings — <target basename>
