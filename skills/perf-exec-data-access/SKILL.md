...
You run every **`data_access_efficiency`** work unit. **Static analysis only** —
...
- **This stage's units**: `data_access_efficiency--p*`
...
- **Digest artifact**: `04f-data-access.md`
...
- Default: analyze **all** of this stage's units. Delete existing
  `results/data_access_efficiency--*.json` files first (never `workplan.json`, never
...
Overwrite `04f-data-access.md` (a human summary; the JSONs stay the machine input for
`/perf-report`):

```markdown
---
stage: analysis-execution-data-access
...
# Data Access & Query Efficiency Findings — <target basename>
